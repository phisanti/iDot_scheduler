...
# Configuration
CONFIG = {"DEAD_VOLUME": 1, "MAX_SHEETS": 4, "VALID_PLATE_SIZES": {96, 384, 1536}}
...
def read_excel_sheets(file_path: Path) -> Dict[str, pd.DataFrame]:
    ...
    try:
        ...
        for sheet in excel_file.sheet_names[: CONFIG["MAX_SHEETS"]]:
            df = pd.read_excel(
                excel_file, sheet_name=sheet, skiprows=1, header=0, index_col=None
            )
...
def assign_channel_numbers(
    row_letter: str, plate_type: int, start_row: str = "A", is_source: bool = True
) -> int:
    """
...
def simplify_input_data(
    melted_data_frames, plate_type: str = "96"
    ...
    min_rows = {key: df["Row"].min() for key, df in melted_data_frames.items()}
    ...
    for key, df in melted_data_frames.items():
        ...
        df["parallel_channel"] = df.apply(
            lambda row: assign_channel_numbers(
                row["Row"], current_plate_type, min_rows[key], key.startswith("source")
            ),
            axis=1,
        )
    ...
    merged_source = melted_data_frames["source_id"].merge(
        melted_data_frames["source_vol"][["Well", "Value"]].rename(
            columns={"Value": "vol"}
        ),
        on="Well",
        how="left",
    ...
    merged_target = melted_data_frames["target_id"].merge(
        melted_data_frames["target_vol"][["Well", "Value"]].rename(
            columns={"Value": "vol"}
        ),
        on="Well",
        how="left",
    ...
    return {"source": merged_source, "target": merged_target}
...
def empty_plate(size: str, value_col_name: str) -> pd.DataFrame:
    ...
    plate_dims = {"96": ("H", 12), "384": ("P", 24), "1536": ("ZF", 48)}
...
def add_headers(
    ...
    headers = [
        ...
        [
            f"DispenseToWaste={str(dispense_to_waste)}",
            "DispenseToWasteCycles=3",
            "DispenseToWasteVolume=1e-7",
            f"UseDeionisation={str(deionisation)}",
            f"OptimizationLevel={optimization}",
...
def read_instructions() -> str:
    ...
    instructions = (
        files(__package__)
        .joinpath("resources/instructions.md")

...
def setup_logging(
    output_folder: Path, base_name: str, debug_mode: bool = False
) -> None:
    """
...
def validate_source_volumes(
    data_dict: Dict[str, pd.DataFrame], dead_vol: float
) -> list:
    """
    Validate volumes by simulating the actual scheduling process.
    This accounts for per-transfer dead volume constraints and well-by-well availability.

    Returns:
        list: List of error messages for transfers that cannot be satisfied
    """
    errors = []

    # Create a copy of source data to simulate volume depletion
    source_simulation = data_dict["source"].copy()
    failed_transfers = []

    # Simulate the scheduling process for each target transfer
    for _, target_row in data_dict["target"].iterrows():
        liquid_name = target_row["Value"]
        required_vol = target_row["vol"]
        target_well = target_row["Target Well"]

        # Find available source wells for this liquid
        available_sources = source_simulation[
            (source_simulation["Value"] == liquid_name)
            & (source_simulation["vol"] >= required_vol + dead_vol)
        ]

        if available_sources.empty:
            # Check what's actually available for better error messaging
            all_sources = source_simulation[source_simulation["Value"] == liquid_name]
            if all_sources.empty:
                failed_transfers.append(
                    {
                        "target_well": target_well,
                        "liquid": liquid_name,
                        "required": required_vol,
                        "error_type": "no_source_wells",
                    }
                )
            else:
                max_available = (all_sources["vol"] - dead_vol).max()
                failed_transfers.append(
                    {
                        "target_well": target_well,
                        "liquid": liquid_name,
                        "required": required_vol,
                        "available": max(0, max_available),
                        "error_type": "insufficient_volume",
                    }
                )
        else:
            # Simulate using the first available source well
            best_source_idx = available_sources.index[0]
            source_simulation.loc[best_source_idx, "vol"] -= required_vol

    # Generate user-friendly error messages
    if failed_transfers:
        # Group errors by type and liquid for cleaner messaging
        no_source_liquids = set()
        insufficient_volume_details = {}

        for failure in failed_transfers:
            if failure["error_type"] == "no_source_wells":
                no_source_liquids.add(failure["liquid"])
            else:
                liquid = failure["liquid"]
                if liquid not in insufficient_volume_details:
                    insufficient_volume_details[liquid] = {
                        "required": failure["required"],
                        "available": failure["available"],
                        "count": 0,
                    }
                insufficient_volume_details[liquid]["count"] += 1

        # Format error messages
        for liquid in no_source_liquids:
            errors.append(f"No source wells found for liquid '{liquid}'")

        for liquid, details in insufficient_volume_details.items():
            errors.append(
                f"Insufficient volume for '{liquid}': "
                f"{details['required']:.2f} μL required per transfer "
                f"but only {details['available']:.2f} μL available per well "
                f"(after {dead_vol} μL dead volume). "
                f"Affects {details['count']} transfer(s)."
            )
...
def validate_volumes(
    source_wells_df: pd.DataFrame, volume: float, liquid_name: str
) -> pd.Series:
    """
    ...
    if wells_above_dead_vol.empty:
        raise ValueError(
            f"No wells for {liquid_name} have more than the dead volume ({dead_vol}uL)"
        )

    valid_wells = wells_above_dead_vol[
        wells_above_dead_vol["Value"] > volume + dead_vol
    ]
    if valid_wells.empty:
        max_available = wells_above_dead_vol["Value"].max() - dead_vol
        raise ValueError(
            f"Requested volume ({volume}uL) for {liquid_name} exceeds maximum available ({max_available:.2f}uL)"
        )
...
def create_worklist_entry(
    source_well: str, target_well: str, transfer_vol: float, liquid_name: str
) -> dict:
    """
...
def update_source_volume(
    data_dict: Dict[str, pd.DataFrame], source_well: str, transfer_vol: float
) -> None:
    """
    ...
    Returns:
        None
    """
    data_dict["source"].loc[data_dict["source"]["Well"] == source_well, "vol"] -= (
        transfer_vol
    )


def process_sequential_transfers(
    data_dict: Dict[str, pd.DataFrame], remaining_targets: pd.DataFrame, dead_vol: float
) -> list:
    """
        ...
        if not valid_source.empty:
            ...
            worklist_entries.append(
                create_worklist_entry(
                    source_well, row["Target Well"], row["vol"], row["Value"]
                )
            )
...
def find_matching_sequence(source_df, target_sequence, target_vols, dead_vol=1):
    ...
    logger.debug(
        f"Starting sequence search with target: {target_sequence}, vols: {target_vols}, dead_vol: {dead_vol}"
    )
    ...
    for col in source_df["Column"].unique():
        ...
        for s_i in range(len(source_data) - window_size + 1):
            for t_i in range(len(target_sequence) - window_size + 1):
                logger.debug(
                    f"Checking window in column {col} at source index {s_i} and target index {t_i}"
                )
                ...
                match_mask = (
                    (id_window["Value"].values == sub_target)
        ...
        if col_matches:
            ...
            logger.debug(
                f"Best window for column {col}: matches={best_window['matches']}, wells={best_window['wells']}"
            )

    if sequences:
        best_col = max(sequences.items(), key=lambda x: x[1]["matches"])[0]
        logger.debug(
            f"Selected best column {best_col} with {sequences[best_col]['matches']} matches"
        )
...
def process_parallel_transfers(
    data_dict: Dict[str, pd.DataFrame], remaining_targets: pd.DataFrame, dead_vol: float
) -> Tuple[list, set]:
    """
    ...
    for channel_i in range(1, max_parallel_channels.max() + 1):
        ...
        for col_i in unique_cols:
            ...
            matching_wells, match_mask = find_matching_sequence(
                data_dict["source"], target_sequence, target_vols, dead_vol
            )
            ...
            if matching_wells:
                ...
                for value, target_well, source_well, transfer_vol in zip(
                    ...
                    worklist_entries.append(
                        create_worklist_entry(
                            source_well, target_well, transfer_vol, value
                        )
                    )
...
def create_iDot_worklist(
    ...
    if use_parallel:
        ...
        while True:
            ...
            logger.info(
                f"Unique parallel channels: {data_dict['target']['parallel_channel'].unique()}"
            )
            parallel_pass_counter += 1
            new_worklist_entries, new_processed_wells = process_parallel_transfers(
                data_dict, remaining_targets, dead_vol
            )
            ...
            remaining_targets = data_dict["target"][
                ~data_dict["target"]["Target Well"].isin(all_processed_wells)
            ]
    ...
    sequential_entries = process_sequential_transfers(
        data_dict, remaining_targets, dead_vol
    )
    ...
    if clean_labels:
        worklist_df["Liquid Name"] = worklist_df["Liquid Name"].str.replace(
            r"-\d+$", "", regex=True
        )
        logger.debug("Labels cleaned of numerical suffixes")

    na_counts = worklist_df[worklist_df["Source Well"].isna()][
        "Liquid Name"
    ].value_counts()
...
def generate_worklist(
    ...
    logger.info(
        f"Running iDot-scheduler version {__version__} on Python {sys.version.split()[0]}"
    )
    ...
    logger.info(
        f"Configuration - Plate size: {plate_size}, Parallelisation: {parallelisation}"
    )
    ...
    if not isinstance(parallelisation, ParallelisationType):
        raise ValueError(
            f"Invalid parallelisation type. Must be one of {[p.value for p in ParallelisationType]}"
        )
    ...
    if volume_errors:
        error_msg = "❌ Volume Validation Failed:\n\n" + "\n".join(
            f"• {error}" for error in volume_errors
        )
    ...
    if parallelisation == ParallelisationType.IMPLICIT:
        idot_wl, na_count = create_iDot_worklist(
            data_dict, use_parallel=True, clean_labels=False
        )
    elif parallelisation == ParallelisationType.EXPLICIT:
        idot_wl, na_count = create_iDot_worklist(
            data_dict, use_parallel=False, clean_labels=True
        )
    else:  # ParallelisationType.NONE
        idot_wl, na_count = create_iDot_worklist(
            data_dict, use_parallel=False, clean_labels=False
        )
    ...
    return (
        f"✅ Worklist generated: {os.path.basename(excel_output)} and {os.path.basename(csv_output)}",
        idot_wl,
    )
